...
export class LiveblocksService {
  ...
  constructor(
    ...
    const liveblocksSecret = this.configService.get<string>('LIVEBLOCKS_SECRET_KEY');
    if (!liveblocksSecret) {
  ...
  async getCurrentUser(authHeader: string): Promise<AuthUser> {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      console.error('Invalid auth header format:', authHeader ? 'Present but wrong format' : 'Missing');
    ...
    try {
      ...
      console.log('User authenticated:', { user_id: data.user_id, role: data.role });
      return data;
    } catch (error) {
   ...
   */
  async checkGroupAccess(user: AuthUser, projectId: string, groupId: string): Promise<boolean> {
    console.log(`Checking access for user ${user.user_id} to project ${projectId}, group ${groupId}`);
    ...
    console.log('Allowing student access (TODO: implement proper group membership check)');
    return true;
  }
   ...
   */
  async generateAccessToken(authHeader: string, projectId: string, groupId: string) {
    console.log(`Generating access token for project ${projectId}, group ${groupId}`);
    ...
    try {
      ...
      console.log('User info:', { name: `${user.first_name} ${user.last_name}`, email: user.email, role: user.role });
