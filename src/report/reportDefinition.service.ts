...
export class ReportDefinitionService {
    ...
    private validateFormatSpecificRules(format: "CLASSIC" | "QUESTIONNAIRE", instruction?: string, questions?: string): void {
        if (format === "CLASSIC") {
            // For CLASSIC format: instruction is mandatory, questions should be null/empty
            if (!instruction || instruction.trim() === '') {
                throw new BadRequestException("For CLASSIC format, instruction field is mandatory");
            }
            if (questions && questions.trim() !== '') {
                throw new BadRequestException("For CLASSIC format, questions field should be null or empty");
            }
        } else if (format === "QUESTIONNAIRE") {
            // For QUESTIONNAIRE format: questions is mandatory, instruction is optional
            if (!questions || questions.trim() === '') {
                throw new BadRequestException("For QUESTIONNAIRE format, questions field is mandatory");
            }
        }
    }

    async create(reportDefinition: CreateReportDefinitionDto): Promise<ReportDefinition> {
        ...
        if (!project) {
            throw new NotFoundException(`Project with id ${reportDefinition.projectId} not found`);
        }
        ...
        try {
            const { projectId, ...reportDefData } = reportDefinition;
            
            // Validate format-specific rules
            this.validateFormatSpecificRules(reportDefData.format, reportDefData.instruction, reportDefData.questions);
            ...
            throw new InternalServerErrorException(
                error.message || "An error occurred while creating the report definition",
            );
        }
    }
    ...
    async findOne(id: string): Promise<ReportDefinition> {
        try {
            ...
            throw new InternalServerErrorException(`Error finding report definition: ${error.message}`);
        }
    }
    ...
    async findByProjectId(projectId: string): Promise<ReportDefinition> {
        try {
            ...
            if (!reportDefinition) {
                throw new NotFoundException(`No report definition found for project '${projectId}'`);
            }
    ...
    async findAll({ limit, page }: ListReportDefinitionsDto): Promise<ReportDefinition[]> {
        try {
            ...
            throw new InternalServerErrorException(`Error fetching report definitions: ${error.message}`);
        }
    }

    async update(id: string, fieldsToUpdate: PatchReportDefinitionDto): Promise<ReportDefinition> {
        ...
        try {
            ...
            const mergedData = {
                ...
                instruction: formattedDto.instruction !== undefined ? formattedDto.instruction : existingReportDefinition.instruction,
                questions: formattedDto.questions !== undefined ? formattedDto.questions : existingReportDefinition.questions,
            };

            // Validate format-specific rules
            this.validateFormatSpecificRules(mergedData.format, mergedData.instruction, mergedData.questions);
            ...
            throw new InternalServerErrorException(`Error updating report definition: ${error.message}`);
        }
    }

    async updateByProjectId(projectId: string, fieldsToUpdate: PatchReportDefinitionDto): Promise<ReportDefinition> {
    ...
    async upsertByProjectId(projectId: string, reportDefinitionData: Omit<CreateReportDefinitionDto, 'projectId'>): Promise<ReportDefinition> {
        // Check if project exists
        const project = await this.projectRepository.findOne({
            where: { id: projectId },
            relations: ["reportDefinition"],
        ...
        try {
            ...
            // Validate format-specific rules
            this.validateFormatSpecificRules(reportDefinitionData.format, reportDefinitionData.instruction, reportDefinitionData.questions);
            ...
            throw new InternalServerErrorException(
                error.message || "An error occurred while upserting the report definition",
            );
        }
    }
    ...
    async delete(id: string): Promise<ReportDefinition> {
        ...
        try {
            ...
            throw new InternalServerErrorException(`Error deleting report definition: ${error.message}`);
        }
    }
} 